...
const frog = (h: number[]): number => {
  // [i - 1]と[i - 2]のコストを比較/メモをしつつ、最小のコストを計算する
  const minCosts: number[] = [0];
  let oneJumpCost: number = 0;
  let twoJumpCost: number = 0;
  ...
  for (let i = 1; i < h.length; i++) {
    if (i === 1) {
      ...
      if (oneJumpCost < twoJumpCost) minCosts.push(oneJumpCost);
      if (twoJumpCost < oneJumpCost) minCosts.push(twoJumpCost);
    };
  }
